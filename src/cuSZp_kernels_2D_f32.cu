...
static __constant__ float profiling_ebs_128[128] = {
    // 1e-1 to 5e-2 range
                                                        ...
                                                        uint blockNum, const uint3 dims, const float eb)
{
    ...
    // Prequantization + Spatial delta + Fixed-length encoding + Store metadata
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        if(block_idx < blockNum)
        {
            ...
            // Processing a 8x8 data block, row-delta.
            #pragma unroll 8
            for(uint i=0; i<8; i++)
            {
                ...
                // No padding row inside a block.
                if(data_idx_y < dims.y)
                {
                    // Vectorized data read.
                    float4 tmp_buffer = reinterpret_cast<const float4*>(oriData)[data_idx/4]; 
                    
                    // Column-wise delta for the first element in a row.
                    // Get quantization, sign, delta, and max absquant, for 0-th element in a row.
                    ...
                    // Get quantization, sign, delta, and max absquant, for 1-th element in a row.
                    ...
                    // Get quantization, sign, delta, and max absquant, for 2-th element in a row.
                    ...
                    // Get quantization, sign, delta, and max absquant, for 3-th element in a row.
                    ...
                    // No partial padding row.
                    if(data_idx_x + 4 < dims.x)
                    {
                        // Vectorized data read.
                        tmp_buffer = reinterpret_cast<const float4*>(oriData)[(data_idx+4)/4];

                        // Get quantization, sign, delta, and max absquant, for 4-th element in a row.
                        currQuant = quantization(tmp_buffer.x, recipPrecision);
                        lorenQuant = currQuant - prevQuant;
                        prevQuant = currQuant;
                        sign_ofs = (i * 8 + 4) % 32;
                        ...
                        // Get quantization, sign, delta, and max absquant, for 5-th element in a row.
                        currQuant = quantization(tmp_buffer.y, recipPrecision);
                        lorenQuant = currQuant - prevQuant;
                        prevQuant = currQuant;
                        sign_ofs = (i * 8 + 5) % 32;
                        ...
                        // Get quantization, sign, delta, and max absquant, for 6-th element in a row.
                        currQuant = quantization(tmp_buffer.z, recipPrecision);
                        lorenQuant = currQuant - prevQuant;
                        prevQuant = currQuant;
                        sign_ofs = (i * 8 + 6) % 32;
                        ...
                        // Get quantization, sign, delta, and max absquant, for 7-th element in a row.
                        currQuant = quantization(tmp_buffer.w, recipPrecision);
                        lorenQuant = currQuant - prevQuant;
                        prevQuant = currQuant;
                        sign_ofs = (i * 8 + 7) % 32;
                    ...
                    // Partial padding row for the last 4 elements.
                    else
                    {
            ...
            // Store metadata, and thread-level prefix-sum.
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        // Retriving index for current data block.
        ...
        if(block_idx < blockNum && fixed_rate[j])
        {
            ...
            for(uint i=0; i<fixed_rate[j]; i++)
            {
                ...
                // Write data to memory in a vectorized way.
                reinterpret_cast<uchar4*>(cmpData)[cmp_byte_ofs/4] = tmp_char1;
                reinterpret_cast<uchar4*>(cmpData)[cmp_byte_ofs/4+1] = tmp_char2;
                cmp_byte_ofs+=8;
                mask <<= 1;
                                                        ...
                                                        uint blockNum, const uint3 dims, const float eb)
{
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        if(block_idx < blockNum)
        {
            // Get fixed-rate, and thread-level prefix-sum.
    ...
    // Read data from global memory and restore shuffled bits.
    unsigned int base_cmp_byte_ofs = base_idx;
    unsigned int cmp_byte_ofs;
    unsigned int tmp_byte_ofs = 0;
    unsigned int cur_byte_ofs = 0;
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        if(block_idx < blockNum && fixed_rate[j])
        {
            ...
            for(uint i=0; i<fixed_rate[j]; i++)
            {
                ...
                // Restore bit-shuffle information.
            ...
            for(uint i=0; i<8; i++)
            {
                ...
                if(data_idx_y < dims.y)
                {
                    // Column-wise de-delta for the first element in a row.
                    // De-delta and get signs for the 0-th elements in this row.
                    sign_ofs = (i * 8) % 32;
                    ...
                    // De-delta and get signs for the 1-th elements in this row.
                    sign_ofs = (i * 8 + 1) % 32;
                    ...
                    // De-delta and get signs for the 2-th elements in this row.
                    sign_ofs = (i * 8 + 2) % 32;
                    ...
                    // No partial padding row, operates for the rest four elements.
                    if(data_idx_x + 4 < dims.x)
                    {
                        // De-delta and get signs for the 4-th elements in this row.
                        sign_ofs = (i * 8 + 4) % 32;
                        ...
                        // De-delta and get signs for the 5-th elements in this row.
                        sign_ofs = (i * 8 + 5) % 32;
                        ...
                        // De-delta and get signs for the 6-th elements in this row.
                        sign_ofs = (i * 8 + 6) % 32;
                        ...
                        // De-delta and get signs for the 7-th elements in this row.
                        sign_ofs = (i * 8 + 7) % 32;
                                                        ...
                                                        uint blockNum, const uint3 dims, const float eb)
{
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        // If outlier encoding, storing outlier. 
        // This branch wll never execute for padding blocks.
        ...
        if(block_idx < blockNum && fixed_rate[j])
        {
            ...
            {
                // Save signs for 8 rows, padding part.
                cmpData[cmp_byte_ofs++] = 0xff & (sign_flag[2*j] >> 24);
                cmpData[cmp_byte_ofs++] = 0xff & (sign_flag[2*j] >> 16);
                cmpData[cmp_byte_ofs++] = 0xff & (sign_flag[2*j] >> 8);
                
                // Save signs and block bit-shuffle, vectorization.
                tmp_char2 = make_uchar4(0, 0, 0, 0);
                ...
                // Assign block bit-shuffle, vectorized padding part.
                tmp_char1 = make_uchar4(0, 0, 0, 0);
                ...
                // Assign block bit-shuffle, scalar padding part.
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        if(block_idx < blockNum && fixed_rate[j])
        {
            ...
            else if(vec_ofs==1)
            {
                ...
                // Retrieve sign, padding part.
                sign_flag[0] = (0xff000000 & (cmpData[cmp_byte_ofs++] << 24)) |
                               (0x00ff0000 & (cmpData[cmp_byte_ofs++] << 16)) |
                               (0x0000ff00 & (cmpData[cmp_byte_ofs++] << 8));

                // Retrieve sign and block bit shuffle, padding for vectorization.
                tmp_char1 = reinterpret_cast<const uchar4*>(cmpData)[cmp_byte_ofs/4];
                tmp_char2 = reinterpret_cast<const uchar4*>(cmpData)[cmp_byte_ofs/4+1];
                ...
                // Padding vector operation for reverse block bit-shuffle.
                ...
                // Padding scalar operation for reverse block bit-shuffle.
...
__global__ void cuSZp_Profiling_kernel_iter_2D_f32(const float* const __restrict__ oriData, 
                                                unsigned char* const __restrict__ cmpData, 
                                                ...
                                                float range)
{
    ...
    int stride = blockNum;
    uint current_block_idx = warp * stride;
    
    const uint dimxBlock = (dims.x + 7) / 8;
    int tmp_quant[4]; 

    for (int j=0; j<4; j++){
        float errorbound = profiling_ebs_128[4 * lane + j] * range; 
        float recipPrecision = 0.5f/(errorbound);        
        
        int maxQuant = 0;
        
        if (current_block_idx < ((dims.y+7)/8 * (dims.x+7)/8)) {
            uint block_idx_y = current_block_idx / dimxBlock;
            uint block_idx_x = current_block_idx % dimxBlock;
            ...
            for(uint i=0; i<8; i++)
            {
                uint data_idx_y = block_idx_y * 8 + i;
                uint data_idx_x = block_idx_x * 8;
                uint data_idx = data_idx_y * dims.x + data_idx_x; 
                
                int prevQuant = 0;
                
                if(data_idx_y < dims.y) {
                    if(data_idx_x + 7 < dims.x) { 
                         float4 v1 = reinterpret_cast<const float4*>(oriData)[data_idx/4];
                         float4 v2 = reinterpret_cast<const float4*>(oriData)[(data_idx+4)/4];
                         
                         int cur = quantization(v1.x, recipPrecision);
                         int delta = cur - prevQuant_y;
                         prevQuant_y = cur; prevQuant = cur;
                         int aq = abs(delta); if (aq > maxQuant) maxQuant = aq;
                         
                         cur = quantization(v1.y, recipPrecision);
                         delta = cur - prevQuant; prevQuant = cur;
                         aq = abs(delta); if (aq > maxQuant) maxQuant = aq;
                         
                         cur = quantization(v1.z, recipPrecision);
                         delta = cur - prevQuant; prevQuant = cur;
                         aq = abs(delta); if(aq>maxQuant) maxQuant=aq;
                         
                         cur = quantization(v1.w, recipPrecision);
                         delta = cur - prevQuant; prevQuant = cur;
                         aq = abs(delta); if(aq>maxQuant) maxQuant=aq;
                         
                         cur = quantization(v2.x, recipPrecision);
                         delta = cur - prevQuant; prevQuant = cur;
                         aq = abs(delta); if(aq>maxQuant) maxQuant=aq;
                         
                         cur = quantization(v2.y, recipPrecision);
                         delta = cur - prevQuant; prevQuant = cur;
                         aq = abs(delta); if(aq>maxQuant) maxQuant=aq;
                         
                         cur = quantization(v2.z, recipPrecision);
                         delta = cur - prevQuant; prevQuant = cur;
                         aq = abs(delta); if(aq>maxQuant) maxQuant=aq;
                         
                         cur = quantization(v2.w, recipPrecision);
                         delta = cur - prevQuant; prevQuant = cur;
                         aq = abs(delta); if(aq>maxQuant) maxQuant=aq;
                    } 
                    else {
                        for(int k=0; k<8; k++) {
                            if(data_idx_x + k < dims.x) {
                                float val = oriData[data_idx + k];
                                int cur = quantization(val, recipPrecision);
                                int delta;
                                if(k==0) {
                                    delta = cur - prevQuant_y;
                                    prevQuant_y = cur;
                                } else {
                                    delta = cur - prevQuant;
                                }
        ...
        int fixed_rate = get_bit_num(maxQuant);
        tmp_quant[j] = (fixed_rate) ? (9 + fixed_rate * 8) : 1;
    }
    
    int4 quant4 = make_int4(tmp_quant[0], tmp_quant[1], tmp_quant[2], tmp_quant[3]);
    reinterpret_cast<int4*>(cmpData)[idx] = quant4;
    __syncthreads();
    locOffset[(warp + 1) * 32 + lane].x = quant4.x;
    locOffset[(warp + 1) * 32 + lane].y = quant4.y;
    locOffset[(warp + 1) * 32 + lane].z = quant4.z;
    locOffset[(warp + 1) * 32 + lane].w = quant4.w;
    ...
    int4 loc_excl_sum = make_int4(0, 0, 0, 0);
    ...
    { 
        ...
        if(status==2)
        {
            loc_excl_sum.x += cmpOffset[lookback * 32 + lane].x;
            loc_excl_sum.y += cmpOffset[lookback * 32 + lane].y;
            loc_excl_sum.z += cmpOffset[lookback * 32 + lane].z;
            loc_excl_sum.w += cmpOffset[lookback * 32 + lane].w;
            __threadfence();
            break;
        }
        if(status==1){
            loc_excl_sum.x += locOffset[lookback * 32 + lane].x;
            loc_excl_sum.y += locOffset[lookback * 32 + lane].y;
            loc_excl_sum.z += locOffset[lookback * 32 + lane].z;
            loc_excl_sum.w += locOffset[lookback * 32 + lane].w;
        } 
        lookback--;
        __threadfence();
    }    
    ...
    if(warp>0){
        ...
        cmpOffset[warp* 32 + lane].x = loc_excl_sum.x;
        cmpOffset[warp* 32 + lane].y = loc_excl_sum.y;
        cmpOffset[warp* 32 + lane].z = loc_excl_sum.z;
        cmpOffset[warp* 32 + lane].w = loc_excl_sum.w;
        __threadfence();
        
        if(warp==gridDim.x-1){ 
            cmpOffset[(warp+1)* 32 + lane].x = cmpOffset[warp* 32 + lane].x + locOffset[(warp+1) * 32 + lane].x;
            cmpOffset[(warp+1)* 32 + lane].y = cmpOffset[warp* 32 + lane].y + locOffset[(warp+1) * 32 + lane].y;
            cmpOffset[(warp+1)* 32 + lane].z = cmpOffset[warp* 32 + lane].z + locOffset[(warp+1) * 32 + lane].z;
            cmpOffset[(warp+1)* 32 + lane].w = cmpOffset[warp* 32 + lane].w + locOffset[(warp+1) * 32 + lane].w;
        }
        __threadfence();
        flag[warp] = 2;
        __threadfence(); 
...
__global__ void cuSZp_Profiling_kernel_iter_2D_outlier_f32(const float* const __restrict__ oriData, 
                                                unsigned char* const __restrict__ cmpData, 
                                                volatile uint4* const __restrict__ cmpOffset, 
                                                volatile uint4* const __restrict__ locOffset, 
                                                volatile int* const __restrict__ flag, 
                                                ...
                                                float range)
{
    ...
    for (int j=0; j<4; j++){
            ...
            for(uint i=0; i<8; i++)
            {
                ...
                if(data_idx_y < dims.y) {
                    if(data_idx_x + 7 < dims.x) { 
                         ...
                         if (aq > maxQuant) maxQuant = aq;
                         if (i==0) outlier = aq; 
                         else if (aq > maxQuan2) maxQuan2 = aq;
                         
                         cur = quantization(v1.y, recipPrecision);
                         delta = cur - prevQuant; prevQuant = cur;
                         aq = abs(delta); if (aq > maxQuant) maxQuant = aq; if (aq > maxQuan2) maxQuan2 = aq;
        ...
        int fr1 = get_bit_num(maxQuant);
        int fr2 = get_bit_num(maxQuan2);
        int outlier_bytes = (get_bit_num(outlier) + 7) / 8;
        
        int size_plain = fr1 ? 9 + fr1 * 8 : 1; 
        int size_outlier = fr2 ? 9 + outlier_bytes + fr2 * 8 : 9 + outlier_bytes;
        
        tmp_quant[j] = (size_plain <= size_outlier) ? size_plain : size_outlier;
    }
