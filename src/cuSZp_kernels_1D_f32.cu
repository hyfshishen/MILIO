...
// // Define the error bound list for 32 error bound
// __constant__ float profiling_ebs[32] = {
//     1e-1f, 8e-2f, 7e-2f, 6e-2f, 5e-2f, 4e-2f, 3e-2f, 2e-2f,
//     1e-2f, 8e-3f, 7e-3f, 6e-3f, 5e-3f, 4e-3f, 3e-3f, 2e-3f,
//     1e-3f, 8e-4f, 7e-4f, 6e-4f, 5e-4f, 4e-4f, 3e-4f, 2e-4f,
//     1e-4f, 8e-5f, 7e-5f, 6e-5f, 5e-5f, 4e-5f, 3e-5f, 2e-5f
// };

// __global__ void cuSZp_Profiling_kernel_1D_f32(const float* const __restrict__ oriData, 
...
//     const int tid = threadIdx.x; // get current thread id 
//     const int bid = blockIdx.x; // get current thread block id
//     const int idx = bid * blockDim.x + tid; // find the global thread index by bid * 32 + tid 
//     const int lane = idx & 0x1f; // find the index inside of the warp by remainder of 0x1f(31)
//     const int warp = idx >> 5; // find the index of the warp by right shift 5 bits (divisble 32)
//     int base_startindex;

//     //float errorbound = powf(10.0f, -1.0f - lane * 0.15f);
//     float errorbound = profiling_ebs[lane]; // each error bound for each thread from 0 to 32 
//     float recipPrecision = 0.5f/(errorbound*range); // Current recipPrecision for the current thread 
//     base_startindex = warp * 32 * sd; 
//     // find the current start data index(for this block) * stride number 
//     int startindex = base_startindex + lane * 32; // data block start index, for each datablock  
//      // warp id * 32 threads * 1 block per threads (32 element per block) + 32 
...
//     // find the quantization number in the current block 
//     #pragma unroll 8
...
//             //if(idx % 8 == 0){
//             //    printf( "eb = %f  current quant=%u  loren quant=%u  max quant=%u  absQuant[quant_chunk_idx]=%u \n",errorbound, currQuant, lorenQuant,maxQuant,absQuant[quant_chunk_idx]);
//             //}
//     }
//     __syncthreads();
    
//     fixed_rate = get_bit_num(maxQuant);


    
//     //cmpData[idx] = (unsigned char)fixed_rate;
//     cmpData[idx] = ((fixed_rate) ? (5+fixed_rate*4) : 1);

//     //////////////////////////////////////////////////////////////
//     //////////////////// Debug info //////////////////////////////
//     //////////////////////////////////////////////////////////////
// /*
//     if (idx == 8) { // print some debug info  
//         printf("startidx=%u endidx=%u\n" ,startindex, endindex );     
//         printf("idx=%d, eb=%f, fixrate_size=%u,  compressed_block_size=%u max_absquant =%u\n\n" ,
//           idx,
//            errorbound, 
//            fixed_rate,   
//            (unsigned)cmpData[idx],
//            maxQuant
//     );
                                                ...
                                                )
{
    ...
    int base_startindex;
    base_startindex = warp * 32 * sd; 
    int startindex = base_startindex + lane * 32; 
    int endindex = startindex + 32;
    int absQuant[32];
    int fixed_rate;
    int currQuant, lorenQuant, prevQuant, maxQuant;
    ...
    fixed_rate = get_bit_num(maxQuant);
    tmp_quant[j] = ((fixed_rate) ? (5 + fixed_rate * 4) : 1);
    ...
    while(lookback>0)
    { 
        //int lookback_loc = lookback * 32 + lane;
        int status;
        do{
    ...
    if(warp>0){
        ...
        if(warp==gridDim.x-1){ 
            //printf("warp %d, lane %d, excl_sum.x=%u, excl_sum.y=%u, excl_sum.z=%u, excl_sum.w=%u\n", 
                //warp, lane, excl_sum.x, excl_sum.y, excl_sum.z, excl_sum.w);
                                                ...
                                                )
{
    ...
    int absQuant[32];
    //int fixed_rate;
    int currQuant, lorenQuant, prevQuant, maxQuant, maxQuan2;
    int quant_chunk_idx;
    int tmp_quant[4];
    int sign_ofs;
    ...
    outlier = (get_bit_num(outlier)+7)/8;
    int temp_rate = 0;
    int temp_ofs1 = fr1 ? 5 + fr1 * 4 : 1;
    int temp_ofs2 = fr2 ? 5 + fr2 * 4 + outlier: 5 + outlier;
    if(temp_ofs1<=temp_ofs2) 
    {        
        temp_rate = temp_ofs1;
    }
    ...
    //int location_number = (warp + 1) * 32 + lane;
