                                                        ...
                                                        uint blockNum, const uint3 dims, const float eb)
{
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        if(block_idx < blockNum)
        {
            ...
            // Processing a 4x4x4 data block, each slice.
            for(uint i=0; i<4; i++)
            {
                ...
                {
                    // Processing a 4x4 slice, each row.
                    for(uint k=0; k<4; k++)
                    {
                        // Row initialization
                                                        ...
                                                        uint blockNum, const uint3 dims, const float eb)
{
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        if(block_idx < blockNum && fixed_rate[j])
        {
            ...
            // De-delta for a 4x4x4 data block and store data back.
            int currQuant, lorenQuant;
            int prevQuant_z = 0;
            for(uint i=0; i<4; i++)
            {
...
__global__ void cuSZp_Profiling_kernel_iter_3D_f32(const float* const __restrict__ oriData, 
                                                unsigned char* const __restrict__ cmpData, 
                                                ...
                                                float range)
{
    ...
    const uint dimxBlock = (dims.x + 3) / 4;
    int tmp_quant[4]; 
    float4 block_cache[16];

    if (current_block_idx < ((dims.z+3)/4 * dimyBlock * dimxBlock)) {
        uint block_stride_per_slice = dimyBlock * dimxBlock;
        uint block_idx_z = current_block_idx / block_stride_per_slice;
        uint block_idx_y = (current_block_idx % block_stride_per_slice) / dimxBlock;
        uint block_idx_x = (current_block_idx % block_stride_per_slice) % dimxBlock;
        for(uint i=0; i<4; i++) {
            uint data_idx_z = block_idx_z * 4 + i;
            if(data_idx_z < dims.z) {
                for(uint k=0; k<4; k++) {
                    ...
                    if(data_idx_y < dims.y) {
                         uint data_idx_x = block_idx_x * 4;
                         uint data_idx = data_idx_z * dims.y * dims.x + data_idx_y * dims.x + data_idx_x;
                         block_cache[i*4+k] = reinterpret_cast<const float4*>(oriData)[data_idx/4];
                    }
                }
            }
        }
            ...
            for(uint i=0; i<4; i++)
            {
                ...
                if(data_idx_z < dims.z)
                {
                    for(uint k=0; k<4; k++)
                    {
                        ...
                        if(data_idx_y < dims.y)
                        {
                            float4 tmp_buffer = block_cache[i*4+k];

                            int currQuant = quantization(tmp_buffer.x, recipPrecision);
                            int lorenQuant;
                            if(k) lorenQuant = currQuant - prevQuant_y; 
                            else {
                                lorenQuant = currQuant - prevQuant_z;   
                                prevQuant_z = currQuant;
                            }
                            prevQuant_y = currQuant;
                            prevQuant_x = currQuant;
                            int aq = abs(lorenQuant);
...
__global__ void cuSZp_Profiling_kernel_iter_3D_outlier_f32(const float* const __restrict__ oriData, 
                                                unsigned char* const __restrict__ cmpData, 
                                                volatile uint4* const __restrict__ cmpOffset, 
                                                volatile uint4* const __restrict__ locOffset, 
                                                volatile int* const __restrict__ flag, 
                ...
                if(data_idx_z < dims.z)
                {
                    for(uint k=0; k<4; k++)
                    {
                        ...
                        if(data_idx_y < dims.y)
                        {
                            ...
                            prevQuant_x = currQuant;
                            aq = abs(lorenQuant);
                            if (aq > maxQuant) maxQuant = aq; if (aq > maxQuan2) maxQuan2 = aq;
