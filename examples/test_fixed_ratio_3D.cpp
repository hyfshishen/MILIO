...
int main() {
    uint3 dims = make_uint3(1024, 1024, 1024); // 1G elements (1024^3)
    size_t nbEle = dims.x * dims.y * dims.z;
    size_t size_bytes = nbEle * sizeof(float);
    
    // Create dummy data: 3D gradient
    std::vector<float> h_in(nbEle);
    float min_val = 0.0f;
    float max_val = 0.0f;
    for(size_t i=0; i<nbEle; ++i) {
        int z = i / (dims.x * dims.y);
        int rem = i % (dims.x * dims.y);
        int y = rem / dims.x;
        int x = rem % dims.x;
        
        h_in[i] = (float)(x + y + z) * 0.01f;
    ...
    printf("Testing cuSZp_fixed_ratio_3D with target ratio = %.2f...\n", targetRatio);
    
    // Sample rate 500 is consistent with other tests
    float absEB = cuSZp_fixed_ratio_3D(d_in, d_cmp, nbEle, &cmpSize, dims, range, 500, targetRatio);
    ...
    // 56.. old logging
    if (actualRatio >= targetRatio * 0.90) { // Allow slightly more slack for 3D complexities
    ...
    cuSZp_decompress_3D_plain_f32(d_dec, d_cmp, nbEle, cmpSize, dims, absEB);
    ...
    // Use standard compress for 3D
    // Signature: (d_oriData, d_cmpBytes, nbEle, cmpSize*, dims, errorBound, stream)
    cuSZp_compress_3D_plain_f32(d_in, d_cmp_std, nbEle, &cmpSize_std, dims, absEB);
    ...
    // --- Test 2: Outlier Mode ---
    printf("\nTesting cuSZp_fixed_ratio_3D OUTLIER Mode with target ratio = %.2f...\n", targetRatio);
    size_t cmpSize_outlier = 0;
    
    float absEB_outlier = cuSZp_fixed_ratio_3D(d_in, d_cmp, nbEle, &cmpSize_outlier, dims, range, 500, targetRatio, CUSZP_MODE_OUTLIER);
    ...
    cuSZp_decompress_3D_outlier_f32(d_dec, d_cmp, nbEle, cmpSize_outlier, dims, absEB_outlier);
    ...
    cuSZp_compress_3D_outlier_f32(d_in, d_cmp_std, nbEle, &cmpSize_std_outlier, dims, absEB_outlier);
