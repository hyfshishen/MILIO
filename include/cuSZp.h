...
void cuSZp_profile(float* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, float range, int sample_rate, cudaStream_t stream = 0);
float cuSZp_fixed_ratio(float* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, float range, int sample_rate, float ratio, cuszp_mode_t mode = CUSZP_MODE_PLAIN, cudaStream_t stream = 0);
float cuSZp_fixed_ratio_2D(float* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, uint3 dims, float range, int sample_rate, float ratio, cuszp_mode_t mode = CUSZP_MODE_PLAIN, cudaStream_t stream = 0);
float cuSZp_fixed_ratio_3D(float* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, uint3 dims, float range, int sample_rate, float ratio, cuszp_mode_t mode = CUSZP_MODE_PLAIN, cudaStream_t stream = 0);
#ifdef __cplusplus
}
#endif
