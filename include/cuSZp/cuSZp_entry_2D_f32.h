...
void cuSZp_profile_2D_plain_f32(float* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, uint3 dims, float range, int sample_rate, cudaStream_t stream = 0);
void cuSZp_profile_2D_outlier_f32(float* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, uint3 dims, float range, int sample_rate, cudaStream_t stream = 0);
